...
public class ProductDao {
	...
	public void addProduct(Product p) throws SQLException {
		...
		runner.update(sql, p.getId(), p.getName(), p.getPrice(),
				p.getCategory(), p.getPnum(), p.getImgurl(), p.getDescription(),p.getUser());
	}
	// 查找所有商品
	...
	//根据上传者查找书籍
	public List<Product> findProductByUser(String user) throws SQLException {
		String sql = "select * from products where user=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		return runner.query(sql, new BeanListHandler<Product>(Product.class), user);
	}
	public Product findProductByName(String name) throws SQLException {
		String sql = "select * from products where name=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		return runner.query(sql, new BeanHandler<Product>(Product.class), name);
	}
...
//		Object obj = new Object[] { (currentPage - 1) * currentCount, currentCount };
	...
	public Product searchBookByName(String searchfield) throws SQLException{
		String sql ="select * from products where name = ?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		return runner.query(sql, new BeanHandler<Product>(Product.class),searchfield);
